...
void Check(State state, StateManager* manager){
    ...
    int i;
    std::vector<State> followup_states;
    
    for(i=0; i<state.carCount(); i++){
        followup_states[followup_states.size()] = state.move_car(i, 0);
        followup_states[followup_states.size()] = state.move_car(i, 1);
    }
    ...
    size_t j;
    for(j=0; j<followup_states.size(); j++) {
        if(followup_states[j].legal(manager)) {
            Check(followup_states[j], manager);
        }
    }
