...
void print_board(char* filename, twoD_array_t *board);
...
int main(int argc, char* argv[]) {
    ...
    print_board("output-0.ppm", board);
    ...
    // print_board("output.ppm", board);
    end_time = get_time();

    /* print timing information. */
    fprintf(stderr, "\nTotal time %g\n", end_time - start_time);
    
    fprintf(stderr, "\nTime excluding initialization %g\n", end_time - init_done_time);
...
/* constructs twoD_array structure. */
twoD_array_t * build_array(twoD_array_t * a, int rows, int cols) {
    int * temp;
